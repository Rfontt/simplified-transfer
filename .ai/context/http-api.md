...
See `docs/openapi.yaml` for the full schemas (`CreateAccountRequest`, `Account`, `Error`).

## Error mapping (HTTP adapter, `error_handler.go`)
- 400 — malformed body (bind failure) or validation: invalid owner_id UUID, empty currency, negative balance.
- 404 — owner referenced by `owner_id` does not exist.
- 409 — account already exists for this owner.
...
- User endpoints (create user) — not yet designed.
