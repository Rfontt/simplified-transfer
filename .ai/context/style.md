...
- **File naming:** PascalCase per file (e.g. `User.go`, `MonetaryAmount.go`, `CreateAccountCommand.go`, `CreateAccountCommandHandler.go`), one concept per file. (Go community default is snake_case — this repo deliberately deviates.)
...
- **HTTP SRP:** thin controllers (bind → call use case → write response); request DTOs in `request/`, response DTOs in `response/` with `NewXResponse(...)` constructors; error mapping in a dedicated `error_handler.go`.
