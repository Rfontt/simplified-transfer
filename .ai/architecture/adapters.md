...
## Position
`internal/adapters/` — implementations of domain/application ports. Currently: `http/` (Gin) and `postgres/` (pgx + goose).

## HTTP (`internal/adapters/http`)
- `router.go` — `NewRouter(createAccount command.CreateAccountUseCase) *gin.Engine`; wires controller + routes (`POST /accounts`). Router takes use-case ports, not concrete handlers.
- `account_controller.go` — thin controller: bind JSON → call use case → write response. No business logic.
- `request/` — request DTOs with gin `binding` tags (`create_account_request.go`).
- `response/` — response DTOs with a `NewXResponse(appResult)` constructor mapping the application result to the wire format (`create_account_response.go`).
...
## Composition
`cmd/simplified-transfer/main.go` (real entry point) wires everything: `config.Load()` → `postgres.Open` → `postgres.Migrate` → `postgres.NewAccountRepository` → `command.NewCreateAccountCommandHandler` → `apphttp.NewRouter` → `router.Run(":" + port)`.

Note: root `main.go` is stale GoLand boilerplate (demo loop); the real entry is `cmd/simplified-transfer/main.go`.

## Tests
- Postgres adapter: `sqlmock` (regex SQL matching, `WithArgs`, `PgError` codes).
